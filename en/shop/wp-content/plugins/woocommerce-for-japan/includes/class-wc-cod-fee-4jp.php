...
//          if(isset($_GET['section']) and $_GET['section']=='wc_gateway_cod'){
    ...
    public function calculate_order_totals( $cart ) {
        ...
        if( !empty( $current_gateway ) && ( empty( $extra_charge_max_cart_value ) || $extra_charge_max_cart_value >= $subtotal ) ) {
            ...
            $extra_charge_amount            = $this->get_extra_charge_amount($current_gateway->id, $subtotal);
...
//                $base_rate  = $tax->get_shop_base_rate();
    ...
    public function get_extra_charge_amount($payment_gateway, $subtotal) {
        $amount = get_option('woocommerce_' . $payment_gateway . '_extra_charge_amount');
        if ($amount) {
            ...
            if ($subtotal <= 10000) {
                ...
                return (($subtotal - 600000) / 100000) * 1080 + 6480;
            }
        }
        return $amount;
    }
